  ...
  try {
    ...
    const unformattedEvents: Event[] = await page.evaluate(() => {
      try {
        ...
        return Array.from(eventList).map((quote) => {
          const dateFrench = quote.querySelector<HTMLAnchorElement>('h1.eventlist-title a')?.innerText.trim() || '';
          const doorShowTime = quote.querySelector<HTMLParagraphElement>('.col.sqs-col-12.span-12 > div:nth-of-type(2) .sqs-html-content p:nth-of-type(4)')?.innerText.trim() || '';
        ...
        writeLog({ error: `Scraper-level error: ${error.message}`, source: "blueskiesturnblack" });
        return [];
      }
    });
    ...
    for (const event of unformattedEvents) {
      try {
        ...
        writeLog({ error: `Event-level error: ${error.message}`, source: "blueskiesturnblack", event });
      }
    }
    return formattedEvents;
  } catch (error) {
    writeLog({ error: `Source-level error: ${error.message}`, source: 'blueskiesturnblack' });
    return [];
  }
}
